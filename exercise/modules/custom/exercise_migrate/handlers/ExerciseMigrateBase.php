...
/**
 * @file
 * Contains \ExerciseMigration.
 */

abstract class ExerciseMigrateBase extends Migration {
  ...
  public function __construct($arguments) {
    ...
    if (!in_array($this->entityType, array('node', 'taxonomy_term'))) {
      throw new Exception('\ExerciseMigration supports only nodes and terms.');
    }
  ...
  protected function getMigrateDirectory() {
    return variable_get('exercise_migrate_directory', FALSE) ? variable_get('exercise_migrate_directory') : drupal_get_path('module', 'exercise_migrate');
  }
}
